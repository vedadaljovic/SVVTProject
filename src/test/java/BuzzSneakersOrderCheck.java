...
public class BuzzSneakersOrderCheck {
    ...
    public void testLoginAndPersonalData() throws InterruptedException {
        ...
        WebElement OrderLink = wait.until(ExpectedConditions.elementToBeClickable(
                By.xpath("//a[@title='Provjeri status porudžbine']")));
        OrderLink.click();

        Thread.sleep(4000);

        WebElement orderNumberInput = wait.until(ExpectedConditions.presenceOfElementLocated(
                By.id("order_number")));
        orderNumberInput.sendKeys("123456789");

        Thread.sleep(2000);

        WebElement emailOrderInput = wait.until(ExpectedConditions.presenceOfElementLocated(
                By.id("order_email")));
        emailOrderInput.sendKeys("vedad.aljovic10@gmail.com");

        Thread.sleep(2000);

        WebElement submitButton = wait.until(ExpectedConditions.elementToBeClickable(
                By.id("view_order_submit")));
        submitButton.click();
